function animating_figure(Link, RobotInfo)
...
for jointloop = 1:size(RobotInfo.ColliJoint, 2)
    ...
    if RobotInfo.PointType(jointloop) == 1
        ...
        if (AutoReportGen)
            T = text(ImpactPosVec(1), ImpactPosVec(2), ImpactPosVec(3), strcat('    ----------------- EE',num2str(RobotInfo.ColliJointIdx(jointloop))), 'Color', P.Color, 'FontSize', 12, 'FontWeight','bold');
        end
    else
        ...
        if RobotInfo.CheckBox(LinkCount)
            ...
            if (AutoReportGen)
                T = text(ImpactPosVec(1), ImpactPosVec(2), ImpactPosVec(3), strcat('     ----------------- R',num2str(RobotInfo.ColliJointIdx(jointloop))), 'Color', P.Color, 'FontSize', 12, 'FontWeight','bold');
            end
        end
    end
end
