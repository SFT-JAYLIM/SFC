function Plot_CRI(UserPath, InputDataPath)
...
try
    ...
    %% STL파일 로드
    STLData = STL2MAT(RobotInfo, InputDataPath);
    ...
    if ~isempty(RobotInfo.RobotModel)
        DHparam = Set_RobotModel(InputDataPath, RobotInfo.RobotModel);
    else
        DHparam.a = [];
        DHparam.d = [];
        DHparam.alpha = [];
    ...
    if ~isempty(RobotInfo.BaseModel)
        BaseDHparam = Set_BaseModel(InputDataPath, RobotInfo.BaseModel);
