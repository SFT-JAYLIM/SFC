function RptGenerator(UserPath, RptDataPath)
...
tic 
    ...
    if isfile(UserInfoPath)
        ...
        if m>9
            DocInfo.Note = char(table2cell(UserInfo(10,2)));
        end
    else
...
%% Define the Common Variable
    ...
    ReportFolder=[UserPath,'\output\Report'];
    ...
    FormPath=RptDataPath;
    isReportFormatExist=isfile([FormPath,'format\Report_Format_test.xlsx']);
...
%% RobotInfo Loading
    ...
    if isRobotInfoFileExist
        ...
        DocInfo.ColliBodyMsg=[];
        msg=[];
        ColliBodyAll=unique(ColliBody);
        [m n]=size(ColliBodyAll);
        
        for i=1:m
                        
            if strcmpi(char(ColliBodyAll(i)),'SkullandForehead')
                  BodyMsg='머리';
            elseif strcmpi(char(ColliBodyAll(i)),'Chest')
                BodyMsg='가슴';
            elseif strcmpi(char(ColliBodyAll(i)),'Upperarmandelbow')
                BodyMsg='상완(삼각근)';
            elseif strcmpi(char(ColliBodyAll(i)),'HandandFinger')
                BodyMsg='손';
            elseif strcmpi(char(ColliBodyAll(i)),'Lowerarmandwrist')
                BodyMsg='하박부 ';
            elseif strcmpi(char(ColliBodyAll(i)),'Lowerarmandwrist')
                BodyMsg='대퇴부';
            end
            ...
            if i==1
                DocInfo.ColliBodyMsg=[DocInfo.ColliBodyMsg, BodyMsg];
            else
                DocInfo.ColliBodyMsg=[DocInfo.ColliBodyMsg, ', ', BodyMsg];
            end
            
            DocInfo.TestItem = ['동적 / ',DocInfo.ColliBodyMsg];
        ...
        while whileIdx
            ...
            switch BodyPPIdx
                ...
                case 6
                    ...
                    if isfile([FormPath,'\format\Report_Format_test.xlsx'])
                        disp('[Status] = Generate the Report Format to publish')
                        Status=copyfile([FormPath,'\format\Report_Format_test.xlsx'],[UserPath,'\output\Report\PFL_REPORT_',DocInfo.RefIdx,'.xlsx']);
                        if Status==1
                 ...
                 %% Solver Result Loading
                    ...
                    if RobotInfo.numColliBody~=0
                        ...
                        for i=1:length(t)
                            tempIdx=[];
                            tempCRIIdx=[];
                            for j=1:RobotInfo.numColliBody
                ...
                %% Image Information Exist Check
                    ...
                    % Velocity Info
                    VelocityImg=[UserPath,'\output\graph',DocInfo.pn,'\All_pos_velocity.jpg'];
                    isVelocityImgExist=isfile(VelocityImg);
                    % Mobile Robot Velocity Info
                    MobileVelocityImg=[UserPath,'\output\graph',DocInfo.pn,'\Mobile_base_velocity.jpg'];
                    isMobileVelocityImgExist=isfile(MobileVelocityImg);
                    % Cobot Velocity Info
                    CobotVelocityImg=[UserPath, '\output\graph',DocInfo.pn,'\Cobot_velocity.jpg'];
                    isCobotVelocityImgImgExist=isfile(CobotVelocityImg);
                    %EE Velocity Info
                    EEVelocityImg=[UserPath, '\output\graph',DocInfo.pn,'\EE_Vel.jpg'];
                    isEEVelVelocityImgImgExist=isfile(CobotVelocityImg);
                    % CRI IMG
                    CRIinfoImg=[UserPath,'\output\graph',DocInfo.pn,'\All_CRI_Res.jpg'];
                    ...
                    for i=1:RobotInfo.numColliBody
                        HSPACEIMGPath=[UserPath, '\output\HumanSpace_',num2str(i),'.jpg'];
                        if isfile(HSPACEIMGPath)
                            disp(['    - Loading the HSpace Image Data ',num2str(i),' / ',num2str(RobotInfo.numColliBody),' Success']);
                        else
                            disp(['    - Loading the HSpace Image Data ',num2str(i),' / ',num2str(RobotInfo.numColliBody),' Failed']);
                        end
                    end


                    % Report file save Path Set
                    excelFileName=[UserPath,'\output\Report\PFL_REPORT_',DocInfo.RefIdx,'.xlsx'];
                    ...
                    if ((contains(DocInfo.Writer,'Shin')) || (contains(DocInfo.Writer,'shin')) || (contains(DocInfo.Writer,'신')))
                        ...
                        NewPath=cd;
                        cd(OldPath)
                        SinatureImg=[NewPath,'\Report_gen\image\signature__SHS.jpg'];
                    elseif ((contains(DocInfo.Writer,'Lim')) || (contains(DocInfo.Writer,'lim')) || (contains(DocInfo.Writer,'임')))
                        OldPath=cd;
                        cd ../..
                        NewPath=cd;
                        cd(OldPath)
                        SinatureImg=[NewPath,'\Report_gen\image\signature__LJH.jpg'];
                    else
                        OldPath=cd;
                        cd ../..
                        NewPath=cd;
                        cd(OldPath)
                        SinatureImg=[NewPath,'\Report_gen\image\Safetics_CI.jpg'];
                        DocInfo.Writer='Safetics. Inc';
                    end
                   ...
                   %% Title Page Generation
                    ...
                    CELLIDX=[66 67 68 69 70 71 72 73 74];
                    AColliBody=split(RobotInfo.ColliBody,'%');

                    for i=1:RobotInfo.numColliBody
                        Cell.ColliBodyHSPACE=[char(CELLIDX(i)),num2str(30)];
                        Cell.Results=[char(CELLIDX(i)),num2str(31)];

                        if strcmpi(char(AColliBody(i)),'SkullandForehead')
                            BodyMsg='머리';
                        elseif strcmpi(char(AColliBody(i)),'Chest')
                            BodyMsg='가슴';
                        elseif strcmpi(char(AColliBody(i)),'Upperarmandelbow')
                            BodyMsg=['상완',char(13),char(10),'(삼각근)'];
                        elseif strcmpi(char(AColliBody(i)),'HandandFinger')
                            BodyMsg='손';
                        elseif strcmpi(char(AColliBody(i)),'Lowerarmandwrist')
                            BodyMsg='하박부 ';
                        else
                        ...
                        %Collibody at HSPACE 
                        eActivesheetRange = get(objExcel.Activesheet,'Range',Cell.ColliBodyHSPACE);
                        eActivesheetRange.Value=BodyMsg;
                        %PASS/FAIL RES
                        eActivesheetRange = get(objExcel.Activesheet,'Range',Cell.Results);
                        eActivesheetRange.Value=PASSFAILMSG;
                    end
                    eActivesheetRange = get(objExcel.Activesheet,'Range','J30');
                    eActivesheetRange.Value=num2str(max(max(MaxModiCRI)),formatSpec);
                    eActivesheetRange = get(objExcel.Activesheet,'Range','J31');
                 ...
                 %% AnalysisCondition1 Page Generation    
                    isRptAnalysisCondition1PageDone=0;
                    VcaptionMsg=[];

                    fprintf(LogfileID, '============== AnalysisCondition1 Generation Start ===============');
                    disp('[Status] = AnalysisCondition1 Generation Start')
                    fprintf(LogfileID, '\r\n');
                    fprintf(LogfileID, '\r\n');
                    fprintf(LogfileID, '[Status] = AnalysisCondition1 Generation Start : %s\n', SimStartCnt);
...
%                     Cell.CaptionForVelocity='A30';
                    ...
                    SheetName='AnalysisCondition1';
                    objExcel.ActiveWorkbook.Worksheets.Item(SheetName).Activate; % set active sheet

                    % Colli. Pos. on robot
                    ColliPosImgOnRobot=ReportImgResize(UserPath, ColliPosImgOnRobot);
                    ...
                    LinkToFile = 0;
                    SaveWithDocument = 1;
                    left = objExcel.ActiveSheet.Range(Cell.ColliPosImgOnRobot).Left+10;
                    ...
                    % Colli. Pos. on EE
                    ColliPosImgOnEE=ReportImgResize(UserPath, ColliPosImgOnEE);
                    ...
                    LinkToFile = 0;
                    SaveWithDocument = 1;
                    left = objExcel.ActiveSheet.Range(Cell.ColliPosImgOnEE).Left+10;
...
%                                                 msg=[msg,num2str(idx),') ',num2str(J3(i)),'번 : '];
                                                ...
                                                if CoverFillet==0
                                                    if OwnFillet==0 % -> Cover : 0, Own : 0, Add~=0
                                                        msgnew=['피복 : ',num2str(ClothFillet),' mm 적용'];
                                                    else % -> Cover : 0, Own ~=0, Add~=0
                                                        msgnew=['자체필렛 : ',num2str(OwnFillet), ', 피복 : ',num2str(ClothFillet),' mm 적용'];
                                                    end

                                                else % Cover~=0
                                                    if OwnFillet==0 % -> Cover ~=0 0, Own : 0, Add~=0
                                                        msgnew=['피복 : ',num2str(ClothFillet),', 커버 : ',num2str(CoverFillet),' mm 적용 '];
                                                    else % -> Cover ~=0, Own ~=0, Add~=0
                                                        msgnew=['자체필렛 : ',num2str(OwnFillet), ', 피복 : ',num2str(ClothFillet),', 커버 : ',num2str(CoverFillet),' mm 적용'];
                                                    end
                                                end

                                                if string(msgold)~=string(msgnew)
                                                    msg=[msg,msgnew,char(13),char(10)]
                                                    msgold=msgnew;
                                                else %msgold == msgnew
%                                                     msg
                                                end
...
%                                             OwnFillet=TotalFillet-(ClothFillet+CoverFillet);
                    ...
                    SimEndCnt=string(datetime('now'));
                    fprintf(LogfileID, '[Status] = AnalysisCondition1 Page Generation End : %s\n', SimEndCnt);
                    fprintf(LogfileID, '[Status] = AnalysisCondition1 Page Generation Sucess');
                    fprintf(LogfileID, '\r\n');
                    fprintf(LogfileID, '\r\n');
                    isRptAnalysisCondition1PageDone=1;
                    disp('[Status] = AnalysisCondition1 Page Generate Success')

                    %% AnalysisCondition2 Page Generation
                    isRptAnalysisCondition2PageDone=0;
                    fprintf(LogfileID, '============== AnalysisCondition2 Page Generation Start ===============');
                    disp('[Status] = AnalysisCondition2 Generation Start')
                    fprintf(LogfileID, '\r\n');
                    fprintf(LogfileID, '\r\n');
                    
                    %Start Time Cnt
                    SimStartCnt=string(datetime('now'));
                    fprintf(LogfileID, '[Status] = AnalysisCondition2 page Generate Start Time : %s\n', SimStartCnt);

                    %Cell Define
                    Cell.HSPACESETIMG='B4';
                    HSPACESETCELLIDX=[29 31 33 35 37 39 41 43 45 47];

                    % Velocity1 sheet
                    SheetName='AnalysisCondition2';
                    objExcel.ActiveWorkbook.Worksheets.Item(SheetName).Activate; % set active sheet

                    HSPACESETIMGPATH=[UserPath, '\output\NumberingHumanSpace.jpg'];

                    if isfile(HSPACESETIMGPATH)
                        HSPACESETIMGPATH=ReportImgResize(UserPath, HSPACESETIMGPATH);
                        imInfo = imfinfo(HSPACESETIMGPATH);
    
                        ImgRealHeight = imInfo.Height*ImgHeightCoeff;
                        ImgRealWidth = imInfo.Width*ImgWidthCoeff;
    
                        CellRealHeight=340*ExcelHeightCoeff;
                        CellRealWidth=81.25*ExcelWidthCoeff;
                        imWidth = imInfo.Width*(CellRealWidth/ImgRealWidth)*CompensationCoeff;
                        imHeight = imInfo.Height*(CellRealHeight/ImgRealHeight)*CompensationCoeff;
                        
                        LinkToFile = 0;
                        SaveWithDocument = 1;
                        left = objExcel.ActiveSheet.Range(Cell.HSPACESETIMG).Left;
                        top = objExcel.ActiveSheet.Range(Cell.HSPACESETIMG).Top+20;
                        objExcel.ActiveSheet.Shapes.AddPicture(HSPACESETIMGPATH,LinkToFile,SaveWithDocument,left,top,imWidth,imHeight);
                    end

                    for i=1:RobotInfo.numColliBody

                        Cell.HSPACESETINFO=['D',num2str(HSPACESETCELLIDX(i))];
                        ...
                        eActivesheetRange = get(objExcel.Activesheet,'Range',Cell.HSPACESETINFO);
                        eActivesheetRange.Value=BodyMsg;
                        eActivesheetRange.font.Color='000000';

                        eActivesheetRange = get(objExcel.Activesheet,'Range',['B',num2str(HSPACESETCELLIDX(i))]);
                        eActivesheetRange.font.Color='000000';
                    end

                    SimEndCnt=string(datetime('now'));
                    fprintf(LogfileID, '[Status] = AnalysisCondition2 Page Generation End : %s\n', SimEndCnt);
                    fprintf(LogfileID, '[Status] = AnalysisCondition2 Page Generation Sucess');
                    fprintf(LogfileID, '\r\n');
                    fprintf(LogfileID, '\r\n');
                    isRptAnalysisCondition1PageDone=1;
                    disp('[Status] = AnalysisCondition2 Page Generate Success')
                    ...
                    switch VelGraphIdx
                        ...
                        case 1 % 1: 모바일 베이스만 있을 때
                            ...
                            eActivesheetRange.Value='  1) 모바일 로봇에 위치한 충돌지점 속도'; 
                        ...
                        case 2 % 2: 협동로봇만 있을 때
                            ...
                            eActivesheetRange = get(objExcel.Activesheet,'Range',Cell.VelocityMsgFirst);
                            eActivesheetRange.Value='  1) 협동로봇에 로봇에 위치한 충돌지점 속도';
                            eActivesheetRange = get(objExcel.Activesheet,'Range',Cell.VelocityMsgSecond);
                            eActivesheetRange.Value='  - 이하여백 -';
                        ...
                        case 3 % 3: EE만 있을 때
                            % EE Velocity
                            eActivesheetRange = get(objExcel.Activesheet,'Range',Cell.VelocityMsgFirst);
                            eActivesheetRange.Value='  1) 끝단부에 위치한 충돌지점 속도'; 
                            eActivesheetRange = get(objExcel.Activesheet,'Range',Cell.VelocityMsgSecond);
                            eActivesheetRange.Value='  - 이하여백 -';
                        ...
                        case 4 % 4: 모바일, 협동로봇만 있을 때
                            ...
                            eActivesheetRange.Value='  1) 모바일 로봇에 위치한 충돌지점 속도'; 
                            eActivesheetRange = get(objExcel.Activesheet,'Range',Cell.VelocityMsgSecond);
                            eActivesheetRange.Value='  2) 협동로봇에 위치한 충돌지점 속도'; 
                        ...
                        case 5 % 5: 모바일과 EE만 있을 때
                            ...
                            eActivesheetRange.Value='  1) 모바일 로봇에 위치한 충돌지점 속도'; 
                            eActivesheetRange = get(objExcel.Activesheet,'Range',Cell.VelocityMsgSecond);
                            eActivesheetRange.Value='  2) 끝단부에 위치한 충돌지점 속도'; 
                        ...
                        case 6  % 6: 협동로봇과 EE만 있을 때
                            eActivesheetRange = get(objExcel.Activesheet,'Range',Cell.VelocityMsgFirst);
                            eActivesheetRange.Value='  1) 협동로봇에 위치한 충돌지점 속도'; 
                            eActivesheetRange = get(objExcel.Activesheet,'Range',Cell.VelocityMsgSecond);
                            eActivesheetRange.Value='  2) 끝단부에 위치한 충돌지점 속도'; 
                            ...
                            LinkToFile = 0;
                            SaveWithDocument = 1;
                            left = objExcel.ActiveSheet.Range(Cell.First).Left+20;
                            top = objExcel.ActiveSheet.Range(Cell.First).Top;
                            objExcel.ActiveSheet.Shapes.AddPicture(CobotVelocityImg,LinkToFile,SaveWithDocument,left,top,imWidth,imHeight);
                            ...
                            LinkToFile = 0;
                            SaveWithDocument = 1;
                            left = objExcel.ActiveSheet.Range(Cell.Second).Left+20;
                            top = objExcel.ActiveSheet.Range(Cell.Second).Top;
                            objExcel.ActiveSheet.Shapes.AddPicture(EEVelocityImg,LinkToFile,SaveWithDocument,left,top,imWidth,imHeight);
                      
                        case 7  % 7: 모바일, 협동로봇, EE 다 있을 때
                            eActivesheetRange = get(objExcel.Activesheet,'Range',Cell.VelocityMsgFirst);
                            eActivesheetRange.Value='  1) 전체 충돌지점 속도'; 
                            eActivesheetRange = get(objExcel.Activesheet,'Range',Cell.VelocityMsgSecond);
                            eActivesheetRange.Value='  2) 끝단부에 위치한 충돌지점 속도'; 
                    ...
                    eActivesheetRange.Value= DocInfo.ColliBodyMsg;
                    ...
                    if cell2mat(isMobileRobot)=='1'
                        ...
                        eActivesheetRange = get(objExcel.Activesheet,'Range',Cell.Hspace);
                        [m n]=size(RobotInfo.Hspace);
                        if n<3
                            eActivesheetRange.Value=['하단 그림의 표현영역 '];
                        else
                    ...
                    RiskSpaceInfoImg=ReportImgResize(UserPath, RiskSpaceInfoImg);
                 ...
                 %% HSPACE Info Page Generation -> HSPACE

                    SimStartCnt=string(datetime('now'));

                    isHSPACEPageDone=0;

                    fprintf(LogfileID, '============== HSPACE Info Page Generation Start ==============');
                    disp('[Status] = HSPACE Info Page Generation Generation Start')
                    fprintf(LogfileID, '\r\n');
                    fprintf(LogfileID, '\r\n');
                    fprintf(LogfileID, '[Status] = HSPACE Info Page Generation Start : %s\n', SimStartCnt);
                    ...
                    for i=1:RobotInfo.numColliBody
                        ...
                        Cell.HSPACENOIDX=['B',num2str(i*5)];
                        Cell.COLLIBODYIDX=['B',num2str(i*5+1)];
                        Cell.DangerPointsNoIDX=['B',num2str(i*5+2)];
                        Cell.CRIATHSPACEIDX=['B',num2str(i*5+3)];

                        Cell.HSPACENO=['E',num2str(i*5)];
                        Cell.COLLIBODY=['E',num2str(i*5+1)];
                        Cell.DangerPointsNo=['E',num2str(i*5+2)];
                        Cell.CRIATHSPACE=['E',num2str(i*5+3)];
                        Cell.HSPACEMSG=['H',num2str(i*5)];
                        Cell.HSPACEIMG=['J',num2str(i*5)];

                        eActivesheetRange = get(objExcel.Activesheet,'Range',Cell.HSPACENOIDX);
                        eActivesheetRange.Value='1. 협동작업 영역 번호:';
                        eActivesheetRange = get(objExcel.Activesheet,'Range',Cell.HSPACENO);
                        eActivesheetRange.Value=num2str(i);

                        eActivesheetRange = get(objExcel.Activesheet,'Range',Cell.COLLIBODYIDX);
                        eActivesheetRange.Value='2. 충돌 신체 부위:';
                        eActivesheetRange = get(objExcel.Activesheet,'Range',Cell.COLLIBODY);
                        eActivesheetRange.Value=BodyMsg;

                        eActivesheetRange = get(objExcel.Activesheet,'Range',Cell.DangerPointsNoIDX);
                        eActivesheetRange.Value='3. 기준 미충족 포인트 발생개수:';
                        eActivesheetRange = get(objExcel.Activesheet,'Range',Cell.DangerPointsNo);
                        eActivesheetRange.Value=num2str(NumofdangerPoints(1,i));

                        eActivesheetRange = get(objExcel.Activesheet,'Range',Cell.CRIATHSPACEIDX);
                        eActivesheetRange.Value='4. 해당영역 최대 CRI:';
                        eActivesheetRange = get(objExcel.Activesheet,'Range',Cell.CRIATHSPACE);
                        eActivesheetRange.Value=num2str(NumofdangerPoints(2,i),formatSpec);

                        HSPACEIMGPath=ReportImgResize(UserPath, [UserPath, '\output\HumanSpace_',num2str(i),'.jpg']);
                        ...
                        if isfile(HSPACEIMGPath)
                            ...
                            CellRealHeight=96*ExcelHeightCoeff;
                            CellRealWidth=15*ExcelWidthCoeff;
                            CompensationCoeff2=CompensationCoeff*0.97;
                            imWidth = imInfo.Width*(CellRealWidth/ImgRealWidth)*CompensationCoeff2;
                            imHeight = imInfo.Height*(CellRealHeight/ImgRealHeight)*CompensationCoeff2;
    
                            topLeftCorner = Cell.HSPACEIMG;
    
                            LinkToFile = 0;
                            SaveWithDocument = 1;
                            left = objExcel.ActiveSheet.Range(topLeftCorner).Left+20;
                            top = objExcel.ActiveSheet.Range(topLeftCorner).Top+3;
                            objExcel.ActiveSheet.Shapes.AddPicture(HSPACEIMGPath,LinkToFile,SaveWithDocument,left,top,imWidth,imHeight);

                            eActivesheetRange = get(objExcel.Activesheet,'Range',Cell.HSPACEMSG);
                            eActivesheetRange.Value=['협동작업영역',char(13),char(10),'위치정보(',num2str(i),')'];

                            % xls borderline, lineshape list
                            ...
                            CellRange=['B',num2str(i*5),':E',num2str(i*5+3)];
                            ...
                            CellRange=['A',num2str(i*5),':P',num2str(i*5+3)];
                            borders = get(objExcel.ActiveSheet.Range(CellRange), 'Borders');
                            ...
                            fprintf(LogfileID, '[Status] = HSPACE Image file missing!!');
                            fprintf(LogfileID, '\r\n');
                            msg='HSPACE Image file missing!!';
                            disp(['[Warnning] = ',msg])
                        end

                        if i==RobotInfo.numColliBody && RobotInfo.numColliBody<6
                            eActivesheetRange = get(objExcel.Activesheet,'Range',['B',num2str((i+1)*5)]);
                            eActivesheetRange.Value='- 이 하 여 백 -';
                        end

                    end                 
                    DoneMsg='HSPACE Info Page Generation Sucess';
                    disp(['[Status] = ',DoneMsg]);

                    SimEndCnt=string(datetime('now'));
                    fprintf(LogfileID, '[Status] = HSPACE Info Page Generation End : %s\n', SimEndCnt);
                    fprintf(LogfileID, ['[Status] = ',DoneMsg]);
                    fprintf(LogfileID, '\r\n');
                    fprintf(LogfileID, '\r\n');
                ...
                %% footer -> Page Numbering
                    ...
                    NumberOfTotalPage=9+SheetNum;

                    for i=1:NumberOfTotalPage
                        objExcel.WorkSheets.Item(i).PageSetup.CenterFooter = [num2str(i),' / ', num2str(NumberOfTotalPage)];
                        objExcel.WorkSheets.Item(i).PageSetup.CenterHeader = '본 보고서는 정식 리포트가 아닌 임시 해석 보고서이므로 결과는 참고 용도로만 활용 부탁드립니다';
                    end
                ...
                %% Terminate
                    ...
                    if isfile(UserInfoPath)
                        ...
                        [m n]=size(UserInfo);
                        if m>9
                            if length(DocInfo.Note)>4
                                filename=['\',DocInfo.Note,'충돌안전계산서('];
                            else
                                filename='\충돌안전계산서(';                                
                            end
...
%                           
    ...
    fclose(LogfileID);
    if isfile([UserPath,'\output\tempImg.jpg'])
        delete([UserPath,'\output\tempImg.jpg']);
    end
    try
