function gSeqData=Graph_Batch(gSeqData)
    ...
    FileInfo=dir(gSeqData.FolderInfo.UserPath);
    IterCnt=0;
    
    for i=1:size(FileInfo,1)
        if contains(FileInfo(i).name,'KHU_Collision_Risk_Analyze_Result.csv')
            IterCnt=IterCnt+1;
        elseif contains(FileInfo(i).name,'ISO_Collision_Risk_Analyze_Result.csv')
            IterCnt=IterCnt+1;
        end
    end
    ...
    try
        ...
        for iterNo=1:2
            ...
            if isfile(gSeqData.FolderInfo.AnalysisResultFile)
            ...
            %     EEVel=ImpactVelNorm(:,1:size(gSeqData.SimInfo.RobotEndEffectorInfo,1));
                ...
                if size(gSeqData.SimInfo.RiskSpaceInfo, 1)~=0       
                    for j=1:size(gSeqData.SimInfo.RiskSpaceInfo, 1)
                        ...
                        ColliBody=string(gSeqData.SimInfo.RiskSpaceInfo(j).ColliBody);
                        ...
                        if max(CRIbyPoints)<1
                            ylim([0 2])
                        else
                            ylim([0 ceil(max(CRIbyPoints).*1.5)])
                        end
                        set(gca,'XTick',(1:1:n));hold on
                        set(gca,'xticklabel',tickString,'fontsize',8,'fontweight','bold')
