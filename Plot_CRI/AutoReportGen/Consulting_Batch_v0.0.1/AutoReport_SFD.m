...
%                         end
                ...
                case 25
                    ...
                    if strcmpi(TaskIdx,'SFD')
                        gSeqData.FolderInfo.ReportPDF=[gSeqData.FolderInfo.UserPath,'\output\Report\PDF\',ReportFileName];
                    elseif strcmpi(TaskIdx,'SFC')
                        if strcmpi(SimInfo.AutoReport.Standard,'KHU')
                            gSeqData.FolderInfo.ReportPDF=[gSeqData.FolderInfo.UserPath,'\output\Report\PDF\','(KOROS)',ReportFileName];
                        elseif strcmpi(SimInfo.AutoReport.Standard,'ISO') 
                            gSeqData.FolderInfo.ReportPDF=[gSeqData.FolderInfo.UserPath,'\output\Report\PDF\','(ISO)',ReportFileName];
                        end
                    end
    ...
    try
        if isfile(gSeqData.FolderInfo.AnalysisInfoFile)
            ...
            if find(contains(Jsonfieldname,'AutoReport'))
                ...
                if strcmpi(TaskIdx,'SFC')
                    if isfile([gSeqData.FolderInfo.UserPath,'\UserInfo.xlsx'])
                        CreatAutoReportField(gSeqData, 1);
                    else
                        CreatAutoReportField(gSeqData, 2);
                    end
                end
            end
